...
| `hiddenstage_bridge/kv_proj_text_delta_300k_from_epoch1_a0p35.pt` | 보유 중인 실험용 text-delta bridge. live render QA 이후 기본 이미지 route에서는 제외 |
| `hiddenstage_bridge/kv_proj_text_exact_v27_alpha35.pt` | 일반 이미지 생성, style tag prompt, sign, label, caption, readable text prompt에 쓰는 v0.1 기본 품질 bridge profile |

`balanced_pose`와 `style_artist` route는 v0.1에서 모두
`kv_proj_text_exact_v27_alpha35.pt`를 사용합니다. 이전 10k style bridge와
300k text-delta bridge는 live render QA에서 기본 품질 기준을 만족하지 못해 기본 라우팅에서 제외했습니다. 이 bridge profile들은
프로토타입 routing choice이며, 별도 평가 없이 promoted model로 설명하면 안 됩니다.
